...
counter = 0
#En utilisant .map, on prend tout les éléments du tableau et on les met dans |x|
@data_set.map {|x|
...
counter = 0
#On map chaque éléments du tableau dans |x| un par un
@data_set.map {|x|
#Chaque élément est transformer en minuscule
  x = x.downcase
#.include? vérifie l'occurence de la string 'aude'
if x.include?('aude')
#S'il y a occurence, on incrémente le compteur
  counter += 1
end
}
#on affiche le résultat du compteur
...
#On définit une méthode qui est l'inverse de la méthode précédente
def is_upper?(c)
 ! is_lower?(c)
end

#On définit une méthode compte les majuscules en premier caractère des éléments du tableau
def countMaj1
#On initialise un compteur
counter1 = 0
#On map chaque éléments du tableau dans |x| un par un
@data_set.map {|x|
#On vérifie si chaque éléments à une majuscule en deuxième caractère
  if is_upper?(x[1])
